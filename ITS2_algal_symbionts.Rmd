...
```
Plots PRO - colored by orange and blue (n0t reordered yet)
```{r} 
library(RColorBrewer)
...
```
Plots PRO - colored by diverging colors for C-dom and D-dom) reordered
...
#964
 #reorder factor for plotting to make plots in order
sample_data(Mcap_964_Pro)$LocSamp <- factor(sample_data(Mcap_972_Pro)$LocSamp, levels = c("Branch tip_V1", 
    "Branch tip_V9",  "Branch mid_V2","Branch mid_V10","Branch base_V3","Branch base_V11", "Plate top_V4","Plate top_V12","Plate edge_V5", "Plate edge_V13","Plate underside_V6", "Plate underside_V14","Lowest_V8","Lowest_V16","Deepest_V7", "Deepest_V15"))
sample_data(Mcap_964_Pro)$SampleNum <- factor(sample_data(Mcap_972_Pro)$SampleNum, levels = c("V1", 
    "V9",  "V2","V10","V3","V11", "V4","V12","V5", "V13","V6", "V14","V8","V16","V7", "V15"))
levels(sample_data(Mcap_964_Pro)$LocSamp)

Mcap_964_PRO_plot<-plot_bar(Mcap_964_Pro, "SampleNum", fill="its2_type_profile", title="Mcap_964_Pro")+
    scale_fill_manual(values=profile.colors)+
  theme_bw()+
   theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())+
   theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))+
...
probCD$dom<- with(probCD, ifelse(
  Prop.C>=0.5,"C", ifelse(
    Prop.C<=0.5, "D","NA"))
  )

  #get meta per colony
col.dom<-data.ra[c(16,26,31,32)]

  prob.CD.col<- col.dom%>% group_by("ColonyDNA.ID" )
